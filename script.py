...
practices = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('Practice condition.xlsx', selection='0:5'),
...
for thisPractice in practices:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if target_text_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > target_text_2.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if pan_record_2.status == NOT_STARTED and t >= 6-frameTolerance:
            ...
            pan_record_2.record(sec=3, block=False)  # start the recording thread
        ...
        if record_notification_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > record_notification_2.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
...
key_resp_4.rt = []
# keep track of which components have finished
trail_StartComponents = [text_6, key_resp_4]
...
thisExp.nextEntry()
# the Routine "trail_Start" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()

# set up handler to look after randomisation of conditions etc
trials = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('conditions.xlsx', selection='0:16'),
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [text1, sound1, text2, sound2, prime_text, prime_sound, target_text, pan_record]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if target_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > target_text.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if pan_record.status == NOT_STARTED and t >= 6-frameTolerance:
            ...
            pan_record.record(sec=3, block=False)  # start the recording thread
